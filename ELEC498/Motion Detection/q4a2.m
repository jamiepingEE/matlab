...
img2=imread('Pentagon.tiff'); %grayscale
...
for r=2:2:2*length(A(:,1))-3
    ...
    for c=1:length(A(1,:)); %throughout entire row
         ...
         if abs(A(k-1,c)-A(k+1,c))/A(k-1,c)>0.3 %different
    ...
    for c=1:length(A(1,:))-1; %throughout entire row
         if abs(A(k-1,c)-A(k-1,c+1))/A(k-1,c)>0.7 %if very different
